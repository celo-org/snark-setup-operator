...
echo 1 | RUST_LOG=info cargo run --release  --bin contribute -- --unsafe-passphrase --keys-path $BASE_DIR/plumo-contributor.keys --coordinator-url http://localhost:8080
echo 1 | RUST_LOG=info cargo run --release  --bin contribute -- --unsafe-passphrase --keys-path $BASE_DIR/plumo-verifier.keys --participation-mode verify --coordinator-url http://localhost:8080
